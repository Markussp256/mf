...
use matrix_traits::{IntoBaseSquareMatrix, MatrixConstructError, MatrixViewSquare, MatrixSquareTryConstruct};
...
macro_rules! impl_skew_or_anti_herm {
    ($name:ident, $name_m:ident, $fn_name:ident, $t:ident $(, $conj:ident)? ) => {
        pub trait $name : MatrixViewSquare+IntoBaseSquareMatrix where Self::T : $t,
        ...
        impl<T   : $t,
             M   : MatrixViewSquare<T=T>+IntoBaseSquareMatrix<Output=Out>,
